...
from app.schemas.request import UpdateResonatorRequestDto
...
def get_resonator_summary(db: Session) -> List[ResonatorSummaryResponseDto]:
...
def get_resonator_detail(db: Session, user_resonator_id: int) -> ResonatorDetailResponseDto:
    ...
    weapon = WeaponDetailDto.from_user_resonator(user_resonator, weapon_image_url)
    stat = ResonatorStatDto.from_final_stat(user_resonator.final_stat)

    return ResonatorDetailResponseDto(
...
def get_resonator_setting(db: Session, user_resonator_id: int) -> ResonatorSettingResponseDto:
    user_resonator = user_resonator_repository.find_by_id(db, user_resonator_id)
    if user_resonator is None:
        raise CustomException(ErrorCode.RESONATOR_NOT_FOUND)
    ...
    weapon = WeaponSettingDto.from_user_resonator(user_resonator, weapon_image_url)
    logger.debug("무기 조회 후 dto로 변환했습니다.")

    return ResonatorSettingResponseDto(nodes=nodes, weapon=weapon)


def create_resonator(db: Session, resonator_profile: UploadFile) -> CreateResonatorResponseDto:
    try:
        # 공명자 프로필 이미지 저장
        storage = get_object_storage_service()
        ...
        db.commit()
        return CreateResonatorResponseDto(resonatorName=rm.name)
    except Exception:
        db.rollback()
        raise


def update_resonator(db: Session, user_resonator_id: int, data: UpdateResonatorRequestDto) -> None:
    try:
