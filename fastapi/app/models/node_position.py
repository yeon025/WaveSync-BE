...
class NodePosition(str, Enum):
    ...
    (예: TOP)이지만, Spring @JsonValue의 code 필드(예: top)와는 다르다. API 응답
    스키마를 만들 때 code 값으로 변환하는 로직(예: Pydantic field_serializer)이
    반드시 필요하다.
    ...
    @property
    def code(self) -> str:
        """Spring @JsonValue의 code 필드 대응 (예: TOP -> top)."""
        return self.value.lower()

    @classmethod
    def from_code(cls, code: str) -> "NodePosition":
        """code 문자열(예: top)을 멤버로 역변환."""
