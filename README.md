...
- 《명조: 워더링 웨이브》의 캐릭터 스펙 조회는 게임 실행과 접속 과정을 필수적으로 요구합니다.
- 단순 정보 조회에도 여러 단계와 로딩 시간이 필요해 접근성이 낮습니다.
- 게임을 실행할 수 없는 환경에서는 스펙 확인이 어렵습니다.

**해결 방안**

- 사용자가 캐릭터 정보를 서비스에 등록하고 저장할 수 있는 환경을 제공합니다.
- 다만 게임 내부 데이터에 직접 접근하는 것은 불가능하므로, 공식 디스코드에 공유되는 프로필 이미지를 입력 수단으로 활용했습니다.
- 이 이미지에서 캐릭터 정보를 추출하고 저장하여, 게임 실행 없이도 언제든 스펙을 조회할 수 있도록 했습니다.
